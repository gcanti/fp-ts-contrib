 ...
 * @since 2.0.0
 */
export const mapWithIndex: <A, B>(f: (i: number, a: A) => B) => (fa: Iterable<A>) => Iterable<B> = (f) => (fa) => ({
  *[Symbol.iterator]() {
 ...
 * @since 0.12.0
 */
export const map: <A, B>(f: (a: A) => B) => (fa: Iterable<A>) => Iterable<B> = (f) => mapWithIndex((i, a) => f(a))
...
const _filterMapWithIndex: FilterableWithIndex1<URI, number>['filterMapWithIndex'] = (fa, f) => ({
  *[Symbol.iterator]() {
    // eslint-disable-next-line functional/no-let
    let i = 0
    // eslint-disable-next-line functional/no-loop-statement
...
export const FilterableWithIndex: FilterableWithIndex1<URI, number> = {
  ...Compactable,
  ...FunctorWithIndex,
  filter: <A, B extends A>(fa: Iterable<A>, f: PredicateWithIndex<number, A> | RefinementWithIndex<number, A, B>) =>
  ...
  partition: <A, B extends A>(fa: Iterable<A>, f: PredicateWithIndex<number, A> | RefinementWithIndex<number, A, B>) =>
    _partitionMapWithIndex(fa, (i, a) =>
      pipe(
        a,
        E.fromPredicate(
...
export const FoldableWithIndex: FoldableWithIndex1<URI, number> = {
  ...
  reduceRightWithIndex: (fa, b, f) => pipe(fa, toReadonlyArray, A.reduceRightWithIndex(b, f))
}
 ...
 * @since 0.12.0
 */
export function takeWhileMapWithIndex<A, B extends A>(f: (i: number, a: A) => O.Option<B>) {
 ...
 * @since 0.12.0
 */
export function takeWhile<A, B extends A>(f: Predicate<A> | Refinement<A, B>): (fa: Iterable<A>) => Iterable<B> {
 ...
 * @since 0.12.0
 */
export function skipWhile<A>(f: Predicate<A>): (fa: Iterable<A>) => Iterable<A> {
